...
(function () {
  ...
  var _disablePlayerNoisyStepsSwitch = Number(
    parameters['Disable Player Noisy Steps Switch ID'] || 0
  );
  var _disableEventsNoisyStepsSwitch = Number(
    parameters['Disable Events Noisy Steps Switch ID'] || 0
  );
  var _terrainTagSE = [
    String(parameters['Terrain Tag 0 SE'] || ''),
  ...
  var _specialTagSE = [
    String(parameters['Special Tag 1 SE'] || ''),
  ...
  var characterNoisySteps_Game_CharacterBase_update =
  ...
  var characterNoisySteps_Game_CharacterBase_setMoveSpeed =
  ...
  Game_Player.prototype.performNoisyStep = function () {
    if (
      !isPlayerNoisyStepsDisabled() &&
      this.hasWalkAnime() &&
      this._noisyStepWaitCount == 0 &&
      this.isMoving()
    ) {
      this.playNoisyStep();
  ...
  Game_Player.prototype.playNoisyStep = function () {
    var pitch = getRandomRangeInt(
      _stepSEPitch - _pitchVariance,
      _stepSEPitch + _pitchVariance
    );
    AudioManager.playSe({
      name: _terrainTagSE[this.terrainTag()],
      volume: _playerStepSEVolume,
      pitch: pitch,
      pan: 0,
    });
  };
  ...
  var characterNoisySteps_Game_Event_initMembers =
  ...
  Game_Event.prototype.performNoisyStep = function () {
    if (
      !isEventsNoisyStepsDisabled() &&
      !this._noisyStepsDisabled &&
      this.isNPC() &&
  ...
  Game_Event.prototype.playNoisyStep = function () {
    var pitch =
      ...
      _sizeTagPitch * this.sizeMultiplier();
    var pan = _panVariance * this.panVarianceOffset($gamePlayer);
    AudioManager.playSe({
      name: _terrainTagSE[this.terrainTag()],
      volume: _eventsStepSEVolume,
      pitch: pitch,
      pan: pan,
    });
  };
